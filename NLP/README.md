...
Скрипт `nlp_workhouse_analysis.py` выполняет комплексный текстовый анализ корпуса объявлений о наборе в так называемые «рабочие дома» — организации, предлагающие трудоустройство, проживание и питание людям в трудной жизненной ситуации. Цель анализа — выявить типичные языковые конструкции, тематическую структуру, повторяющиеся клише и характерные приёмы этих объявлений.

Скрипт заменяет исходный ноутбук из ~25 ячеек и ~10 классов, которые зависели от тяжёлых библиотек (pymystem3, natasha, dostoevsky, BERTopic). Новая версия — один файл, один класс, минимум зависимостей, полностью рабочий код.
